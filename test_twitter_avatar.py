#!/usr/bin/env python3
"""
Тестирование скачивания аватарок через Twitter движок
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('twitter_avatar_test.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_twitter_avatar_download():
    """Тестирует скачивание аватарок через Twitter движок"""
    ...
    try:
        ...
        logger.info("Начинаем тестирование Twitter движка для скачивания аватарок...")
        ...
        for username in test_usernames:
            ...
            try:
                ...
                if avatar_path and Path(avatar_path).exists():
                    ...
                    logger.info(f"УСПЕХ для @{username}: {avatar_path} ({file_size} байт)")
                else:
                    results[username] = {
                        'success': False,
                        'path': None,
        ...
        # Выводим результаты
        logger.info("\nРЕЗУЛЬТАТЫ ТЕСТИРОВАНИЯ TWITTER ДВИЖКА:")
        logger.info("=" * 60)
...
def check_downloaded_avatars():
    """Проверяет скачанные аватарки"""
    
    logger.info("\nПроверяем скачанные аватарки...")
...
def main():
    ...
    logger.info("Запуск тестирования Twitter движка для аватарок")
    logger.info("=" * 70)
    
    # Проверяем существующие аватарки
    check_downloaded_avatars()
    
    # Тестируем Twitter движок
    results = test_twitter_avatar_download()
    ...
    if results:
        ...
        if success_rate >= 80:
            ...
            logger.info("2. Проверьте работу Twitter syndication API")
            logger.info("3. Возможно нужно обновить селекторы HTML")
